  ...
  const fetchShipments = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setTotal((data.data?.shipments || []).length || 0);
      }
    } catch (error) {
    ...
    <div className="min-h-screen bg-slate-50">
      <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        <div className="mb-8 flex items-center justify-between">
          <div>
            <h1 className="text-4xl font-bold text-slate-900">Gestion des Expéditions</h1>
            <p className="mt-2 text-slate-600">Groupage vers Abidjan</p>
          </div>
          <Link
                ...
                <table className="w-full">
                  <thead className="border-b border-slate-200 bg-slate-50">
                    <tr>
                      ...
                      <th className="px-6 py-3 text-left text-sm font-semibold text-slate-900">Quantité</th>
                      <th className="px-6 py-3 text-left text-sm font-semibold text-slate-900">Montant Total</th>
